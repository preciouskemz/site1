...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (hamburger && overlay) {
        ...
        // Close when clicking a link in overlay
...
function updateBanner() {
    ...
    if (opponentLogo) {
        opponentLogo.src = (match.homeLogo === null) ? '../All Asset/assets/logo.png' : match.homeLogo;
    }
...
// 3D Image Slider - MOUSE FOLLOW
function rotateSlider3d(direction) {
    var carousel = document.getElementById('slider3dCarousel');
    if (!carousel) return;
    var currentRotate = carousel.getAttribute('data-rotate');
    if (!currentRotate) currentRotate = 0;
    currentRotate = parseInt(currentRotate) + direction * 60;
    carousel.style.transform = 'rotateY(' + currentRotate + 'deg)';
    carousel.setAttribute('data-rotate', currentRotate);
}

// Mouse follow effect on 3D carousel
document.addEventListener('mousemove', function(e) {
    var carousel = document.getElementById('slider3dCarousel');
    var wrapper = document.querySelector('.slider3d-wrapper');
    if (!carousel) return;
    
    var rect = wrapper ? wrapper.getBoundingClientRect() : { left: 0, width: window.innerWidth };
    var centerX = rect.left + rect.width / 2;
    var mouseX = e.clientX;
    var offset = (mouseX - centerX) / (rect.width / 2);
    var rotate = offset * 20;
    
    var currentRotate = carousel.getAttribute('data-rotate');
    if (!currentRotate) currentRotate = 0;
    currentRotate = parseInt(currentRotate);
    
    carousel.style.transform = 'rotateY(' + (currentRotate + rotate) + 'deg)';
});

// Auto-rotate 3D slider - starts after mouse leaves
var sliderAutoRotate;
function startAutoRotate() {
    clearInterval(sliderAutoRotate);
    sliderAutoRotate = setInterval(function() {
        rotateSlider3d(1);
    }, 5000);
}
...
// ============================================
// SECTION 2C: MATCH BANNER SCROLL ANIMATION
// ============================================
(function() {
...
// ============================================
// SECTION 2C: MATCH BANNER COUNTDOWN
// ============================================
(function() {
    var targetDate = new Date('2026-04-09T15:00:00+01:00').getTime();
...
// ============================================
function toggleFuturisticResults(gender) {
    var mensTable = document.getElementById('futuristicMens');
    var womensTable = document.getElementById('futuristicWomens');
    var buttons = document.querySelectorAll('.futuristic-results .tab-btn');
    
    for (var i = 0; i < buttons.length; i++) {
        buttons[i].classList.remove('active');
        buttons[i].style.background = 'transparent';
        buttons[i].style.color = 'rgba(255, 255, 255, 0.7)';
    }
    
    if (gender === 'mens') {
        if (mensTable) mensTable.style.display = 'block';
        if (womensTable) womensTable.style.display = 'none';
        buttons[0].classList.add('active');
        buttons[0].style.background = 'linear-gradient(135deg, #22c55e, #16a34a)';
        buttons[0].style.color = 'white';
    } else {
        if (mensTable) mensTable.style.display = 'none';
        if (womensTable) womensTable.style.display = 'block';
        buttons[1].classList.add('active');
        buttons[1].style.background = 'linear-gradient(135deg, #22c55e, #16a34a)';
